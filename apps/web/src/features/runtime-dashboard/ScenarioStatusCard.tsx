    ...
    <StatusCard title="剧本状态" status={panel.status}>
      {panel.status === "success" && scenario ? (
        <dl className="status-list">
          ...
          <div>
            <dt>朝代</dt>
            <dd>{scenario.dynastyName}</dd>
          </div>
          <div>
            <dt>开始日期</dt>
            <dd>{scenario.startGameDate}</dd>
          </div>
          <div>
            <dt>数据完整度</dt>
            <dd>{scenario.historicalDataCompleteness}</dd>
          </div>
          <div>
            <dt>Schema 校验</dt>
            <dd className="positive">{scenario.schemaValidated ? "通过" : "未通过"}</dd>
          </div>
        </dl>
      ) : (
